...
public class NoticeController {
    ...
    @GetMapping("/{courseId}")
    public ResponseEntity<RespCommonInfo<List<RespNoticeDto>>> getNoticeInfoByWeek(@PathVariable int courseId, @RequestParam(required = true) int week) {

        int studentId = 1; // 임시 하드 코딩
        List<RespNoticeDto> noticeList = noticeService.findNoticeInfoByWeek(courseId, studentId, week);
